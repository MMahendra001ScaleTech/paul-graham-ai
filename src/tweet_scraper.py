...
MAX_TWEETS = 10000  # Total number of tweets to scrape
TWEET_LIMIT = 99  # Number of tweets per request (Twitter API allows up to 100)
...
def fetch_tweets(user_id, max_tweets, tweet_limit):
    """Function to fetch tweets of a user by user ID"""
    tweets_list = []
    url = f"{BASE_URL}/users/{user_id}/tweets"
    params = {
        "max_results": tweet_limit,  # Limit the number of tweets per request
        "tweet.fields": "created_at,public_metrics",  # Fields to fetch for each tweet
    }

    try:
        with tqdm(total=max_tweets, desc="Fetching tweets", unit="tweet") as pbar:
            while len(tweets_list) < max_tweets:
                ...
                if response.status_code == 200:
                    ...
                    pbar.update(len(tweets))  # Update progress bar
        ...
        with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
            json.dump(tweets_list[:max_tweets], f, indent=4)

        print(f"✅ Successfully fetched {len(tweets_list[:max_tweets])} tweets and saved to {OUTPUT_FILE}")
...
if __name__ == "__main__":
    ...
    if user_id:
        fetch_tweets(user_id, MAX_TWEETS, TWEET_LIMIT)